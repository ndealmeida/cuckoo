...
class Indexer:
    ...
    def index_elasticsearch(self):
        ...
        logger.info(f"Indexing Elasticsearch index '{index_name}'...")
        ...
        for doc in SAMPLE_DOCUMENTS:
            es.index(index=index_name, id=str(doc["id"]), document=doc)
        es.indices.refresh(index=index_name)

    def index_vespa(self):
        logger.info("Feeding data to Vespa...")
        # get_app() returns the existing connection
        app = self.vespa_service.get_app()
        for doc in SAMPLE_DOCUMENTS:
            app.feed_data_point(schema="doc", data_id=str(doc["id"]), fields=doc)
